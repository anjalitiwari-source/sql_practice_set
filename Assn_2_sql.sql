...
INSERT INTO Customers (CustomerID, CustomerName, PhoneNo, City, AccountType, AccountNo) VALUES
...
SELECT TOP 3 c.CustomerID,c.CustomerName,c.City,a.Balance
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
ORDER BY a.Balance DESC;

--3. Show all customers who have taken loans along with loan amount and loan type.
select c.CustomerID,c.CustomerName,c.City,l.LoanID,l.LoanAmount,l.loanType
...
select sum(Amount) as total_Withdraw from Transactions
...
max(t.Amount) AS hightest_Amount
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
JOIN Transactions t 
ON a.AccountID = t.AccountID
...
SELECT c.CustomerID,c.CustomerName,
COUNT(t.TransactionID) AS total_transaction_each
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
JOIN Transactions t 
ON a.AccountID = t.AccountID
...
select c.CustomerID,c.CustomerName,c.AccountNo,a.balance,
RANK() over(order by a.balance desc) as Customer_rank
from Customers c join Accounts a
on c.CustomerID = a.CustomerID; 
...
select c.CustomerID,c.CustomerName,c.AccountNo,a.balance,
dense_rank() over(order by a.balance desc) as Customer_rank
from Customers c join Accounts a
on c.CustomerID = a.CustomerID;

--12. Show previous transaction amount using LAG() function.
SELECT  c.CustomerID,c.CustomerName,t.Amount,t.TransactionDate,
LAG(t.Amount) OVER ( ORDER BY t.TransactionDate) AS Previous_Transaction_Amount
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
JOIN Transactions t 
ON a.AccountID = t.AccountID;
...
SELECT  c.CustomerID,c.CustomerName,t.Amount,t.TransactionDate,
lead(t.Amount) OVER ( ORDER BY t.TransactionDate) AS Next_Transaction_Amount
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
JOIN Transactions t 
ON a.AccountID = t.AccountID;
...
SELECT   c.CustomerID, c.CustomerName, t.Amount, t.TransactionDate,
SUM(t.Amount) OVER ( ORDER BY t.TransactionDate) AS Running_Total
FROM Customers c JOIN Accounts a 
ON c.CustomerID = a.CustomerID
JOIN Transactions t 
ON a.AccountID = t.AccountID;
...
with balance_rank as (select c.CustomerID,c.CustomerName,a.AccountID,a.balance,
dense_RANK() over( order by a.balance desc) as rank_num
from Customers c join Accounts a 
on c.CustomerID = a.CustomerID
)
SELECT CustomerID,CustomerName,AccountID,Balance FROM balance_rank
WHERE rank_num = 2;
